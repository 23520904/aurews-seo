...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div className="wired-wrapper">
      <header style={{ marginBottom: '64px', borderBottom: '2px solid var(--wired-black)', paddingBottom: '32px' }}>
        ...
        <form action="/search" style={{ marginTop: '32px', display: 'flex', gap: '16px' }}>
          <input 
            ...
            style={{ flex: 1, padding: '16px', border: '2px solid var(--wired-black)', fontSize: '24px', fontFamily: 'var(--font-display)', outline: 'none' }} 
          />
      ...
      {query ? (
        <div>
          <h2 className="wired-mono" style={{ fontSize: '14px', marginBottom: '32px' }}>RESULTS FOR: "{query.toUpperCase()}" — FOUND {results.length}</h2>
